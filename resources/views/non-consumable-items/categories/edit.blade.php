...
<script>
    ...
    name.addEventListener('change', function() {
        fetch('/non-consumable-items/categories/checkSlug?name=' + name.value)
...
<x-layout>
    <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
        <div class="breadcrumb-title pe-3 fw-semibold">Ubah Data - {{ $category->name }}</div>
        <div class="ps-3">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0 p-0">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">
                        <a href="{{ route('non-consumable-items.categories', $unit->slug) }}">Data Kategori</a>
                    </li>
                </ol>
            </nav>
        </div>
                    ...
                    <div class="row g-3">
                        <div class="col-12">
                            <div class="card shadow-none bg-light border">
                                <div class="card-body">
                                    <form class="row g-3"
                                        action="{{ route('non-consumable-items.categories.update', [$unit->slug, $category->category_slug]) }}" method="POST">
                                        @csrf
                                        @method('PUT')
                                        <div class="col-12">
                                            ...
                                            <input type="text" class="form-control @error('category_code') is-invalid @enderror" name="category_code"
                                                placeholder="Kode Kategori" value="{{ old('category_code', $category->category_code) }}">
                                            @error('category_code')
                                        ...
                                        <div class="col-12">
                                            ...
                                            <input type="text" class="form-control @error('category_name') is-invalid @enderror" name="category_name"
                                                id="category_name" placeholder="Nama Kategori" value="{{ old('category_name', $category->category_name) }}">
                                            @error('category_name')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                        </div>
                                        <div class="col-12">
                                            ...
                                            <input type="text" class="form-control @error('category_slug') is-invalid @enderror" name="category_slug"
                                                id="category_slug" placeholder="Slug Kategori" value="{{ old('category_slug', $category->category_slug) }}"
                                                readonly>
                                            @error('category_slug')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                        ...
                                        <div class="text-end mt-4">
                                            ...
                                            <a href="{{ route('non-consumable-items.categories', $unit->slug) }}" class="btn btn-secondary fw-semibold">
                                                KEMBALI
                                            </a>
                                        </div>
                                    </form>
