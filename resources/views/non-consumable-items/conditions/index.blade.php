...
<x-layout>
    <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
        <div class="breadcrumb-title pe-3 fw-semibold">Data Kondisi</div>
        <div class="ps-3">
        ...
        <div class="ms-auto">
            <div class="btn-group">
                <a href="{{ route('non-consumable-items.conditions.create', $unit->slug) }}" class="btn btn-success">Tambah Data</a>
            </div>
        </div>
    </div>
    <hr />
                ...
                <table id="dataTable" class="table table-striped align-middle" style="width:100%">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <h6 class="mb-0 product-title text-black">{{ $condition->name }}</h6>
                            </td>
                            <td>{{ $condition->slug }}</td>
                            <td>
                                <div class="table-actions d-flex align-items-center gap-3 fs-4">
                                    <a href="{{ route('non-consumable-items.conditions.edit', [$unit->slug, $condition->slug]) }}" class="text-warning"
                                        data-bs-toggle="tooltip" data-bs-placement="bottom" title="Ubah Data">
                                        <i class="bi bi-pencil-fill"></i>
                                    </a>
                                    <form action="{{ route('non-consumable-items.conditions.delete', [$unit->slug, $condition->slug]) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
