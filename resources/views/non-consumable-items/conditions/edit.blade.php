...
<x-layout>
    <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
        <div class="breadcrumb-title pe-3 fw-semibold">Ubah Data - {{ $condition->name }}</div>
        <div class="ps-3">
                    ...
                    <div class="row g-3">
                        <div class="col-12">
                            <div class="card shadow-none bg-light border">
                                <div class="card-body">
                                    <form class="row g-3" action="{{ route('non-consumable-items.conditions.update', [$unit->slug, $condition->slug]) }}"
                                        method="POST">
                                        @csrf
                                        @method('PUT')
                                        <div class="col-12">
                                            ...
                                            <input type="text" class="form-control @error('name') is-invalid @enderror" name="name" id="name"
                                                placeholder="Nama Kondisi" value="{{ old('name', $condition->name) }}">
                                            @error('name')
                                        ...
                                        <div class="col-12">
                                            ...
                                            <input type="text" class="form-control @error('slug') is-invalid @enderror" name="slug" id="slug"
                                                placeholder="Slug Kondisi" value="{{ old('slug', $condition->slug) }}" readonly>
                                            @error('slug')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                        </div>
