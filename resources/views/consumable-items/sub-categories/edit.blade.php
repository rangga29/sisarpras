                    ...
                    <div class="row g-3">
                        <div class="col-12">
                            <div class="card shadow-none bg-light border">
                                <div class="card-body">
                                    <form class="row g-3"
                                        action="{{ route('consumable-items.sub-categories.update', [$unit->slug, $sub_category->sub_category_slug]) }}"
                                        method="POST">
                                        @csrf
                                        @method('PUT')
                                        <div class="col-12">
                                            <label for="cons_category_id" class="form-label">Kategori</label>
                                            <input type="hidden" name="old_cons_category_id" value="{{ $sub_category->cons_category_id }}">
                                            <select class="form-select single-select" name="cons_category_id" data-width="100%">
                                                @foreach ($categories as $category)
                                                @if(old('cons_category_id', $sub_category->cons_category_id) == $category->id)
