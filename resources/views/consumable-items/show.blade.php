            ...
            <div class="row">
                                ...
                                <table class="table table-bordered table-hover align-middle mb-0">
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>{{ $item->cons_sub_category->cons_category->category_name }}</td>
                                        </tr>
                                        <tr>
                                            <td class="fw-semibold">Sub Kategori Barang</td>
                                            <td>{{ $item->cons_sub_category->sub_category_name }}</td>
                                        </tr>
                                        <tr>
                                        ...
                                        <tr>
                                            <td class="fw-bolder">Jumlah Awal</td>
                                            <td>{{ $item->initial_amount }}</td>
                                        </tr>
                                        <tr>
                                            <td class="fw-bolder">Jumlah Diambil</td>
                                            <td>{{ $item->taken_amount }}</td>
                                        </tr>
                                        <tr>
                                            <td class="fw-bolder">Jumlah Stok</td>
                                            <td>{{ $item->stock_amount }}</td>
                                        </tr>
                                        <tr>
                                            <td class="fw-bolder">Tanda Terima Barang</td>
                                            <td><a href="{{ asset('storage/consumable-items/' . $item->receipt) }}" target="_BLANK">
                                                    Tanda Terima Barang
                                                </a>
                                            </td>
                                        </tr>
                ...
                <div class="col-12 col-lg-4">
                    <div class="card border shadow-none bg-light radius-10">
                        <div class="card-body">
                            <img src="{{ asset('storage/consumable-items/' . $item->image) }}" alt="{{ $item->item_code }}" class="w-100">
                        </div>
                    </div>
                    <div class="card border shadow-none bg-light radius-10">
                        <div class="card-body">
                            <div class="d-grid text-nowrap gap-2">
                                <a href="{{ route('consumable-items.edit', [$unit->slug, $item->item_code]) }}" class="btn btn-sm btn-warning mb-2">
                                    <h6 class="mt-2">
                                        <i class="bi bi-pencil-fill"></i> UBAH DATA
                                    </h6>
                                </a>
                                <form action="{{ route('consumable-items.delete', [$unit->slug, $item->item_code]) }}" method="POST" class="d-inline">
                                    @method('DELETE')
                                    @csrf
                                    <div class="d-grid">
